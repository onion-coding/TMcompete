...
class User(Base):
    ...
    ubisoft_id = Column(String(100), unique=True, nullable=False)  # Ubisoft unique ID
    username = Column(String(100), nullable=False)                 # Ubisoft display name
    email = Column(String(100), nullable=True)                     # optional
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    competitions = relationship("Competition", backref="creator")
